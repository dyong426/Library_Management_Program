...
public class AdminDao extends MenuDao{
	...
	@Override
	public ArrayList<AdminVO> get2(int header, String searchStr) throws SQLException {

		StringBuilder sql = new StringBuilder(selectSql2(header));
	...
	public StringBuilder selectSql2(int header) {
		StringBuilder sql = new StringBuilder();
		String phone = "SELECT * FROM admins WHERE admin_phone = ?";
		String email = "SELECT * FROM admins WHERE admin_email = ?";
		if (header == 1) {
			sql.append(phone);
		} else if (header == 2) {
			sql.append(email);
